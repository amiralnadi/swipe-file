...
export default function BoardContent() {
  ...
  // Redirect if viewing someone else's board
  useEffect(() => {
    if (
  ...
  }, [sessionStatus, session, username, router]);

  const { isLoading, isNotInstalled, isNeedsRepo } = useOnboard();
  const { items, folders, isLoading: itemsLoading, mutate } = useItems();

  const [activeFolder, setActiveFolder] = useState<string | null>(searchParams.get("f"));
  const [activeCategory, setActiveCategory] = useState<string | null>(searchParams.get("c"));
  ...
  const pushUrl = useCallback(
    (folder: string | null, category: string | null) => {
      const p = new URLSearchParams();
      if (folder) p.set("f", folder);
  ...
  const handleCategoryClick = useCallback(
    (folderSlug: string, category: string | null) => {
      setActiveFolder(folderSlug);
      setActiveCategory(category);
      setActiveTag(null);
      setVisibleCount(12);
      pushUrl(folderSlug, category);
    },
    [pushUrl]
  );
      ...
      <div className="flex flex-col items-center justify-center min-h-screen gap-6 px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            Install the MySwipe.cc app on GitHub to connect your swipe file. You can limit access to
            just one repository.
          </p>
          <a
      ...
      <div className="flex flex-col items-center justify-center min-h-screen gap-6 px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            ...
            <code className="bg-stone-100 px-1.5 py-0.5 rounded text-xs font-mono">swipe-file</code>{" "}
            on GitHub, then grant the app access to it.
          </p>
